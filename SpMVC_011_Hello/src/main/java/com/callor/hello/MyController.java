...
public class MyController {
	 ...
	 * - 전송하는 데이터가 HTTP 프로토콜의 BODY 에 담겨서 전송되기 때문에
	 * 		일단 주소창 등에 노출이 되지 않는다.
	 * - 이론상으로는 전송하는 데이터의 크기에 제한이 없다.
	 * 		다만, 이미지와 같이 큰 데이터를 전송하게 되면 네트워크에 문제가 발생할 수 있어서
	 ...
	 * method=POST 만 있을 경우 서버는 405 HTTP 상태코드(Status Code)를 응답한다.
	 */
	@RequestMapping(value = "/", method = RequestMethod.POST)
	public void my() {
	}
	
	// insert URI mapping 으로 POST 방식의 요청을 처리하겠다.
	@RequestMapping(value = "/insert", method = RequestMethod.POST)
	public void insert() {
	}

	/*
	 * Spring에서는 Web으로부터 전달받은 데이터를 Java method에 매개변수를 설정하여 받을 수 있다.
	 * 
	 ...
	 * 
	 * 그런데,
	 * Java method의 변수를 String 형이 아닌 것으로 (숫자형, Boolean형 등) 설정을 하면,
	...
	// insert URI mapping 으로 GET 방식의 요청을 처리하겠다.
