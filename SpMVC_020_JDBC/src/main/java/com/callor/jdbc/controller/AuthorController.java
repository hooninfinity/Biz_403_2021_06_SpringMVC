...
public class AuthorController {
	...
	public String list(HttpSession hSession, Model model) {
		...
		/*
		 * Attribute를 지정할 때 흔히 저지르는 실수
		 * Attribute의 이름은 문자열 형태이기 때문에 어떠한 기호를 사용해도 무방하다.
		 * 하지만,
		 * jsp에서 렌더링할 때에는 이 Attr문자열이 변수로 취급이 된다.
		 * 변수로 취급된다는 것은 java의 일반적인 코딩규칙이 적용된다는 것이다.
		 * 
		 * 작성할때는 비록 문자열이지만, 변수명명규칙에 맞도록 이름을 작성해야 한다.
		 * 그렇지 않으면 jsp에서 오류를 만나게 될 것이다.
		 */
		model.addAttribute("AUTH_LIST", auList); // 속성을 지정할때 대문자로 해놓는건 쌤만의 패턴
		
		return "author/list";
	}
	
	/*
	 * cp_title을 Request로부터 받아 변수에 세팅을 하는데
	 * Req를 할때 au_name 변수를 보내지 않으면 400 httpStatus 오류가 발생한다.
	 * 400 오류는 서버 App 디버깅 과정에서 상당히 관리하기 어려운 오류이다.
	 * 
	 * 단순한 변수(VO, DTO, MAP 형식이 아닌 단일 변수)일 경우는
	 * @RequestParam의 required 옵션을 false로 선언하고, 
	 * default값을 임의로 설정해두면 코드내에서 핸들링을 할 수 있다.
	 */
