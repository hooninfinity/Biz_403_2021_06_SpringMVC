 ...
 * 목적
 * 1. 상속되는 부모클래스의 역할
 * 		스스로는 객체로 생성될 수 없다.(인터페이스와 같음)
 * 2. 추상메서드를 포함하는데
 * 		상속받은 곳에서는 반드시 추상메서드를 구현해야 한다.(인터페이스와 같음)
 * 3. 상속받은 클래스들에 필요한 공통처리 method는 일반 클래스처럼 자체적으로 구현 코드를 가지고 있다.(일반클래스의 성질)
 * 4. 공통처리 method도 필요에 따라 상속받는 곳에서 재 정의 할 수 있다.(일반클래스의 성질)
 * 
 * 5. 인터페이스의 역할을 수행
